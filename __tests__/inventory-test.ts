...
describe('Inventory', () => {
	...
	it('should parse buc state', () => {
		expect(toInventoryItem({...EMPTY_ITEM, str: 'a blessed potion of extra healing'}).buc).toEqual(BUCState.BLESSED);
		expect(toInventoryItem({...EMPTY_ITEM, str: 'a uncursed potion of extra healing'}).buc).toEqual(BUCState.UNCURSED);
		expect(toInventoryItem({...EMPTY_ITEM, str: 'a cursed potion of extra healing'}).buc).toEqual(BUCState.CURSED);
	});
