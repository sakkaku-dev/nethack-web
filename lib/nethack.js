...
["_main","_shim_graphics_set_callback","_mapglyph","_display_inventory","_fflush","___set_stack_limits","onRuntimeInitialized"].forEach((prop) => {
...
var ASM_CONSTS = {
  1806578: () => { globalThis.nethackGlobal = globalThis.nethackGlobal || {}; globalThis.nethackGlobal.constants = globalThis.nethackGlobal.constants || {}; },  
 1806720: () => { globalThis.nethackGlobal = globalThis.nethackGlobal || {}; globalThis.nethackGlobal.globals = globalThis.nethackGlobal.globals || {}; }
};
function js_helpers_init() { globalThis.nethackGlobal = globalThis.nethackGlobal || {}; globalThis.nethackGlobal.helpers = globalThis.nethackGlobal.helpers || {}; installHelper(mapglyphHelper); installHelper(displayInventory); installHelper(getPointerValue); installHelper(setPointerValue); installHelper(createPointerValue); function mapglyphHelper(glyph, x, y, mgflags) { let ochar = _malloc(4); let ocolor = _malloc(4); let ospecial = _malloc(4); _mapglyph(glyph, ochar, ocolor, ospecial, x, y, mgflags); let ch = getValue(ochar, "i32"); let color = getValue(ocolor, "i32"); let special = getValue(ospecial, "i32"); _free (ochar); _free (ocolor); _free (ospecial); return { glyph, ch, color, special, x, y, mgflags }; } function displayInventory() { return _display_inventory(0, 0); } function getPointerValue(name, ptr, type) { switch(type) { case "s": return UTF8ToString(ptr); case "p": if(!ptr) return 0; return getValue(ptr, "*"); case "c": return String.fromCharCode(getValue(ptr, "i8")); case "0": return getValue(ptr, "i8"); case "1": return getValue(ptr, "i16"); case "2": case "i": case "n": return getValue(ptr, "i32"); case "f": return getValue(ptr, "float"); case "d": return getValue(ptr, "double"); case "o": return ptr; default: throw new TypeError ("unknown type:" + type); } } function createPointerValue(name, type, value) { if(!Array.isArray(value)) throw new TypeError(`expected ${name} return type to be array`); if(value.length == 0 || !Array.isArray(value[0])) throw new TypeError(`expected ${name} return type to be array or contain values`); let int_byte_size = 4; let size = value.length * int_byte_size; let p = _malloc(size); for (i = 0; i < value.length; i++) { let row = value[i]; let start = p + i * size; for (j = 0; j < row.length; j++) { let v = row[j]; setPointerValue(name, start + j * int_byte_size, 'i', v) } } return p; } function setPointerValue(name, ptr, type, value = 0) { switch (type) { case "p": throw new Error("not implemented"); case "s": if(typeof value !== "string") throw new TypeError(`expected ${name} return type to be string`); stringToUTF8(value, ptr, 1024); break; case "i": if(typeof value !== "number" || !Number.isInteger(value)) throw new TypeError(`expected ${name} return type to be integer`); setValue(ptr, value, "i32"); break; case "c": if(typeof value !== "number" || value < 0 || value > 128) throw new TypeError(`expected ${name} return type to be integer representing an ASCII character`); setValue(ptr, value, "i8"); break; case "f": if(typeof value !== "number" || isFloat(value)) throw new TypeError(`expected ${name} return type to be float`); setValue(ptr, value, "double"); break; case "d": if(typeof value !== "number" || isFloat(value)) throw new TypeError(`expected ${name} return type to be double`); setValue(ptr, value, "double"); break; case "v": break; default: throw new Error("unknown type"); } function isFloat(n){ return n === +n && n !== (n|0) && !Number.isInteger(n); } } function installHelper(fn, name) { name = name || fn.name; globalThis.nethackGlobal.helpers[name] = fn; } }
...
var ___stop_em_js = Module['___stop_em_js'] = 1806578;
